	...
	tests := []struct {
	...
	}{
		{
			...
			want: getMetrics(
				getSumMetric(
					...
					"",
					true,
					getAttributes("key_name", value71, label12, value12),
					1.,
		...
		},
		{
			...
			want: getMetrics(
				getSummaryMetric(
					value61,
					getAttributes("key_name", value61, label12, value12),
					uint64(time.Now().UnixNano()),
					2.,
					2.,
					getQuantiles(),
		...
		},
		{
			...
			want: getMetrics(
				getDoubleGaugeMetric(
					value81,
					getAttributes("key_name", value81, label12, value12),
					2.,
					uint64(time.Now().UnixNano()))),
			wantErr: assert.NoError,
		},
...
func getQuantiles() pmetric.SummaryDataPointValueAtQuantileSlice {
